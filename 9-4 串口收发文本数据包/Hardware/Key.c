...
void Key_Init(void)
{
	...
	GPIO1_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO1_InitStructure.GPIO_Pin = GPIO_Pin_14;
	GPIO1_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO1_InitStructure);						//将PB1和PB11引脚初始化为上拉输入
}
...
uint8_t Key_GetNum(void)
{
	...
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_15) == 0)			//读PB1输入寄存器的状态，如果为0，则代表按键1按下
	{
		Delay_ms(20);											//延时消抖
	...
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_11) == 0)			//读PB11输入寄存器的状态，如果为0，则代表按键2按下
	{
    	...
    	if (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_14) == 0)			//读PB11输入寄存器的状态，如果为0，则代表按键2按下
	{
		Delay_ms(20);											//延时消抖
